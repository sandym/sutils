...
private:
	...
	bool moveToFrontOfQueue( std::list<job_ptr> &io_queue,
	                         const job_ptr &i_job );
	...
	/*!
	   @brief finish a job right away.
	       ...
	       -If it's waiting in the async queue, it will be removed and executed
	               right away, the completion will be executed as well and then
	   it will be deleted. -If it's executing async, it will block until it
	   finished, then the completion will be executed as well and then it will
	   be deleted. -If it's waiting in the idle queue, it will be removed,
	   executed and deleted right away. MUST be called from the main thread.
	*/
