...
public:
	...
	flat_set() = default;
	explicit flat_set( const value_compare &comp ) : _pair( {}, comp ) {}
...
#endif
	explicit flat_set( const allocator_type &a ) : _pair( a, {} ) {}
#if 0
	...
	{
		auto it =
		    std::lower_bound( begin(), end(), v, [this]( auto &a, auto &b ) {
			    return key_comp()( a, b );
		    } );
		if ( it == end() )
		{
	...
	void sort()
	{
		std::sort( storage().begin(),
		           storage().end(),
		           [this]( auto &a, auto &b ) { return key_comp()( a, b ); } );
	}
