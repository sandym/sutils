...
inline bool isSeparator( char c ) { return c == '/' or c == '\\'; }
#else
const char k_separators[] = {'/', 0};
const char k_illegalChars[] = {0};
const size_t k_minimumAbsolutePathLength = 1;
inline bool isSeparator( char c ) { return c == '/'; }
#endif
...
/*!
   @brief Return true if a path is absolute.
	   ...
	   On windows, consider x:/ paths and unc path. On other platform, return true just for string starting with a separator.
...
inline bool isAbsolute( const su::string_view &i_path )
{
	...
	if ( i_path.length() < k_minimumAbsolutePathLength ) // unc or local path must be at least x char long ("c:\" or "\\s")
		return false;
	if ( isalpha( i_path[0] ) and i_path[1] == ':' and isSeparator( i_path[2] ) )
		return true; // "c:\"
...
std::string CFURLToPath( CFURLRef i_url )
{
	...
	if ( i_url != nullptr )
	{
		su::cfauto<CFStringRef> pathRef( CFURLCopyFileSystemPath( i_url, kCFURLPOSIXPathStyle ) );
...
{
	auto state = sysdir_start_search_path_enumeration( i_sysdir, SYSDIR_DOMAIN_MASK_USER );
...
filepath::filepath( location i_folder )
{
	...
	switch ( i_folder )
	{
		...
		{
		#if UPLATFORM_WIN
			if ( SHGetFolderPathW( 0, CSIDL_PERSONAL, 0, SHGFP_TYPE_CURRENT, buffer ) == 0 )
		...
		case location::kApplicationFolder:
		{
			...
			if ( bundleRef != 0 )
			{
				cfauto<CFURLRef> bundleURL( CFBundleCopyBundleURL( bundleRef ) );
		...
		#else
			memset( buffer, 0, PATH_MAX ); // whatch out, readlink does NOT set a null terminator it seems...
		...
		#elif UPLATFORM_WIN
			if ( SHGetFolderPathW( 0, CSIDL_COMMON_APPDATA, 0, SHGFP_TYPE_CURRENT, buffer ) == 0 )
				_path.assign( su::to_string(buffer) );
		#else
		...
		#elif UPLATFORM_WIN
			if ( SHGetFolderPathW( 0, CSIDL_DESKTOPDIRECTORY, 0, SHGFP_TYPE_CURRENT, buffer ) == 0 )
				_path.assign(su::to_string(buffer));
		#else
...
filepath::filepath( const BookmarkData &i_fileBookmark, const filepath *i_relativeTo )
{
	...
	if ( i_fileBookmark.size() < sizeof(kPathTag) - 1 + k_minimumAbsolutePathLength )
		return;
	if ( strncmp( i_fileBookmark.data(), kPathTag, sizeof(kPathTag) - 1 ) != 0 )
		return;

#if UPLATFORM_MAC || UPLATFORM_IOS
	// on mac, we might have an optional "ALIAS:" tag as well
	size_t pathPartLen = strlen( i_fileBookmark.data() );
	if ( i_fileBookmark.size() > ( pathPartLen + sizeof(kAliasTag) ) and
		 strncmp( i_fileBookmark.data() + pathPartLen + 1, kAliasTag, sizeof(kAliasTag) - 1 ) == 0 )
	{
...
bool filepath::getBookmarkData( BookmarkData &o_data, const filepath *i_relativeTo ) const
{
	...
	if ( i_relativeTo != nullptr )
	{
		...
		if ( not relativeTo.empty() )
		{
			...
			std::pair<decltype(pathComp)::iterator,decltype(pathComp)::iterator> mismatchPoint;
			if ( pathComp.size() > relativeToComp.size() )
			{
				mismatchPoint = std::mismatch( pathComp.begin(), pathComp.end(), relativeToComp.begin() );
				std::swap( mismatchPoint.first, mismatchPoint.second );
			}
			else
				mismatchPoint = std::mismatch( relativeToComp.begin(), relativeToComp.end(), pathComp.begin() );
			
			the_path.clear();
			auto goUp = std::distance( mismatchPoint.first, relativeToComp.end() );
			for ( ; goUp > 0; --goUp )
				the_path += "../";
	...
	cfauto<CFURLRef> relativeToURL( i_relativeTo != nullptr ? createURLRef( i_relativeTo->ospath() ) : nullptr );
	cfauto<CFErrorRef> error;
	cfauto<CFDataRef> bm( CFURLCreateBookmarkData( nullptr,
													url,
...
#if UPLATFORM_WIN
	auto attrs = GetFileAttributesW( ospath().c_str() );
	return (attrs != INVALID_FILE_ATTRIBUTES) and (attrs&FILE_ATTRIBUTE_DIRECTORY);
#else
	struct stat info;
	return (stat( ospath().c_str(), &info ) == 0) and ((info.st_mode&S_IFDIR)!=0);
#endif
}
...
#if UPLATFORM_WIN
	auto attrs = GetFileAttributesW( ospath().c_str() );
	return (attrs != INVALID_FILE_ATTRIBUTES) and not (attrs&FILE_ATTRIBUTE_DIRECTORY);
#else
	struct stat info;
	return (stat( ospath().c_str(), &info ) == 0) and ((info.st_mode&S_IFREG)!=0);
#endif
}
...
bool filepath::isFolderEmpty() const
{
	...
	if ( not empty() )
	{
		...
		if ( fsIterator != INVALID_HANDLE_VALUE )
		{
			do
			{
				if ( wcscmp( info.cFileName, L"." ) != 0 and wcscmp( info.cFileName, L".." ) != 0 )
					foundOne = true;
			}
	...
	if ( d != nullptr )
	{
		...
		{
			if ( strcmp( result->d_name, "." ) != 0 and strcmp( result->d_name, ".." ) != 0 )
				foundOne = true;
		}
		closedir( d );
	}
...
std::vector<filepath> filepath::folderContent() const
{
	...
	if ( not empty() )
	{
		...
		if ( fsIterator != INVALID_HANDLE_VALUE )
		{
			...
			{
				if ( wcscmp( info.cFileName, L"." ) != 0 and wcscmp( info.cFileName, L".." ) != 0 )
					l.emplace_back( _path + "/" + su::to_string(info.cFileName) );
			}
...
bool filepath::up()
{
	...
	while ( _path.length() > k_minimumAbsolutePathLength and isSeparator( _path.back() ) )
		_path.pop_back();
	return true;
}
...
std::ifstream &filepath::fsopen( std::ifstream &o_fs, std::ios_base::openmode i_mode ) const
{
	o_fs.open( ospath().c_str(), i_mode );
	return o_fs;
}

std::ofstream &filepath::fsopen( std::ofstream &o_fs, std::ios_base::openmode i_mode ) const
{
	o_fs.open( ospath().c_str(), i_mode );
	return o_fs;
}

std::fstream &filepath::fsopen( std::fstream &o_fs, std::ios_base::openmode i_mode ) const
{
	o_fs.open( ospath().c_str(), i_mode );
	return o_fs;
}
...
bool filepath::mkdir() const
{
	...
	return true;
#else
	auto err = ::mkdir( ospath().c_str(), S_IRUSR|S_IWUSR|S_IXUSR|S_IRGRP|S_IXGRP|S_IROTH|S_IXOTH );
	if ( err != 0 )
	{
...
bool filepath::move( const filepath &i_dest ) const
{
	...
	if ( i_dest.exists() )
	{
		log_error() << "attempt to move a file to an already existing destination";
		return false;
	}
