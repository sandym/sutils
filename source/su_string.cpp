...
{
    return convert_utf<su_std::string_view,std::wstring>::convert( s );
}
...
{
    return convert_utf<su_std::string_view,std::u16string>::convert( s );
}
...
su_std::string_view trimSpaces_view( const su_std::string_view &i_s )
{
	...
	if ( f != su_std::string_view::npos )
	{
		assert( l != su_std::string_view::npos );
		return i_s.substr( f, l - f + 1 );
	}
	else
		return su_std::string_view();
}
