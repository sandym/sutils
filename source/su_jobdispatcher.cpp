...
jobdispatcher::~jobdispatcher()
{
	...
	_asyncQueue.remove_if( []( const job_ptr &j ){ return j->_threadIndex == -1; } );
...
bool jobdispatcher::removeFromQueue( std::list<job_ptr> &io_queue, const job_ptr &i_job )
{
...
bool jobdispatcher::moveToFrontOfQueue( std::list<job_ptr> &io_queue, const job_ptr &i_job )
{
	auto it = std::find( io_queue.begin(), io_queue.end(), i_job );
	if ( it != io_queue.end() )
	{
