...
su::NameURI extractNameURI( const char *s )
{
	...
	if ( p != end )
		return su::NameURI{ su_std::string_view( p+1, end-(p+1) ), su_std::string_view( s, p-s ) };
	else
		return su::NameURI{ su_std::string_view( s, end-s ), su_std::string_view() };
}
...
void startElement( void *userData, const char *name, const char **atts )
{
	...
	su::flat_map<su::NameURI,su_std::string_view> attribs;
...
void characters( void *userData, const char *s, int len )
{
	...
	if ( not _this->characters( su_std::string_view( s, len ) ) )
		XML_StopParser( parser, false );
}
...
bool saxparser::characters( const su_std::string_view &i_text )
{
	return true;
}
