...
class MyParser : public su::saxparser
{
	public:
		...
		virtual bool startElement( const su::NameURI &i_nameURI,
										const su::flat_map<su::NameURI,su_std::string_view> &i_attribs )
		{
		...
		virtual bool characters( const su_std::string_view &i_s )
		{
			su_std::string_view s( i_s );
		...
		struct Elem
		{
			Elem( const su_std::string_view &i_localname,
					const su_std::string_view &i_URI,
					const su::flat_map<su::NameURI,su_std::string_view> &i_attribs )
